...
function getDiagnosticsSummaryText(
	diagnostics?: SubagentFailureDiagnostics,
): string | null {
...
function getMessagePreview(message: Message): string | undefined {
	for (const part of message.content) {
		...
		if (part.type === "thinking")
			return normalizePreviewText(part.thinking || "");
		if (part.type === "toolCall")
			return normalizePreviewText(`toolCall:${part.name}`);
	}
	return undefined;
}
			...
			const processLine = (line: string) => {
				...
				if (eventType === "message_end" && eventMessage) {
					...
					if (msg.role === "assistant") {
						...
						const runtimeProvider = (msg as Message & { provider?: string })
							.provider;
						if (runtimeProvider)
							currentResult.runtimeProvider = runtimeProvider;
						if (msg.stopReason) currentResult.stopReason = msg.stopReason;
						if (msg.errorMessage) currentResult.errorMessage = msg.errorMessage;

						for (const part of msg.content) {
							if (part.type === "toolCall") {
								const argsPreview = clipText(
